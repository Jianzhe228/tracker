...
`src-tauri/src/db/mod.rs` 为准，`PRAGMA user_version = 1`。
本版本被视为**全新设计的第一版**：全新数据库由单一基线 schema 在一个事务内一次建成，
不存在迁移阶梯，也不兼容任何更早构建产生的数据库文件（启动时直接报错拒绝）。
今后变更 schema 的方式：把变更合并进基线 + 写一个"上一版本→新版本"的单步迁移 + 版本号 +1。
`docs/tracker_schema.sql` 是历史导出，不代表当前结构。
