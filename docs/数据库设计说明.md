...
`src-tauri/src/db/mod.rs` 中的迁移逻辑为准，最新 `PRAGMA user_version = 22`。
自 v22 起不再保留 v1→v21 的增量迁移阶梯：全新数据库由单一基线 schema 一次建成；
唯一支持的升级路径是 21→22（清除旧 AI 预测管线的遗留行），更早版本的数据库直接报错拒绝打开。
`docs/tracker_schema.sql` 是较早的 v3 导出，不代表当前完整结构。
...
```mermaid
graph LR
  Core["核心任务域<br/>projects / tasks / tags / recurring_rules"]
  Time["专注与统计域<br/>focus_sessions / focus_session_segments / task_completion_logs / daily_summaries"]
  ...
  System["系统辅助域<br/>user_settings / notification_logs / ai_logs / task_deletion_logs"]
...
graph TD
  ...
  Tag["tags"] --> TaskTag["task_tags<br/>多对多"]
  Task --> TaskTag
```
...
graph TD
  ...
  Job -. "job_id / source_job_id 逻辑关联" .-> Feedback
```
...
graph LR
  ...
  Pending -. "source_job_id 逻辑关联" .-> Job["ai_jobs"]
```

说明：

- `task_creation_history` 是预测用快照，不外键引用 `tasks`，这样任务删除后仍可保留行为样本。
- `pending_predictions.project_id`、`source_job_id` 是逻辑关联字段，没有外键约束，避免项目或 AI 任务清理影响预测历史。
...
导入时会先清空相关业务表，再按依赖顺序导入上述白名单数据。需要注意：`tags`、`task_tags`、
`notification_logs`、`ai_logs`、`daily_summaries`、`task_deletion_logs`、`suggestion_runs`、
`suggestion_candidates`、`task_creation_history`、`pending_predictions` 当前不在 JSON 同步白名单内；
其中部分表会在导入前被清空，属于“本地运行态/预留/历史审计”数据，而不是完整跨设备同步数据。
...
| `tasks` | 任务、子任务、归档和软删除 | `parent_id` 自引用；`status` 表示 todo/in_progress/done/cancelled；`deleted_at` 支持软删除；`completed_at` 支持归档游标 |
| `recurring_rules` | 重复任务模板 | 保存重复类型、锚点日期、自定义星期、上次生成日期；生成出的任务回写 `tasks.recurring_rule_id` |
| `tags` | 标签基础表 | 当前建表存在，但命令层没有完整标签管理功能，属于预留/未充分使用表 |
| `task_tags` | 任务-标签多对多 | 复合主键 `(task_id, tag_id)`，删除任务或标签时级联清理 |
...
| `daily_summaries` | 日汇总缓存 | 表已存在，当前统计更多直接从 `tasks` 和 `focus_sessions` 聚合，属于预留缓存表 |
...
| `pending_predictions` | 待确认预测 | 保存预测标题、原因、日期、状态、算法版本、分数和去重 key |
...
| `ai_logs` | 旧 AI 建议日志 | 建表存在，当前主要 AI 流程已转向 `ai_skills`/`ai_jobs`/建议追踪表 |
...
| `task_tags.task_id` | `tasks.id` | 真实外键 | `ON DELETE CASCADE` |
| `task_tags.tag_id` | `tags.id` | 真实外键 | `ON DELETE CASCADE` |
...
| `pending_predictions.source_job_id` | `ai_jobs.id` | 逻辑关联 | 无数据库约束 |
...
| `idx_task_tags_task_id` / `idx_task_tags_tag_id` | `task_tags` | 标签关联查询 |
...
1. ~~迁移实现按版本顺序执行，但没有把每个版本统一包进显式事务。~~ 已解决：v22 起基线建库与升级路径均包在显式事务中，失败即整体回滚并阻止应用启动。
...
4. `tags`、`task_tags`、`daily_summaries`、`ai_logs` 已建表但当前业务使用较少，说明 schema 有预留和历史遗留，需要后续清理或补齐功能。
...
当前数据库设计对一个本地优先的专注任务桌面应用是有效的：主业务表规范化程度足够，任务和专注统计的查询路径有索引支撑，AI 学习表使用追加日志和快照避免破坏核心业务数据。它的主要技术债集中在迁移事务化、预留表清理、逻辑关联一致性和 JSON 字段可查询性上。

短期维护建议：

1. ~~为后续迁移增加显式事务，并在迁移失败时停止应用启动。~~ 已在 v22 基线重构中完成。
2. 重新生成当前版本的 schema 导出，替换或归档旧 `tracker_schema.sql`。
3. 标注或删除长期不用的 `ai_logs`、`daily_summaries`、标签相关表。
