...
fn read_skill_row(row: &rusqlite::Row) -> rusqlite::Result<AiSkillRow> {
  ...
  let action_types: Vec<String> =
    serde_json::from_str(&action_types_json).unwrap_or_default();
...
fn read_job_row(row: &rusqlite::Row) -> rusqlite::Result<AiJobRow> {
  ...
  let input_context: serde_json::Value =
    serde_json::from_str(&input_context_str).unwrap_or(serde_json::Value::Object(Default::default()));
...
pub fn ai_skill_create(
  ...
  let db = state.db().lock().map_err(|e| e.to_string())?;
  let action_types_json =
    serde_json::to_string(&payload.action_types.unwrap_or_default())
      .map_err(|e| e.to_string())?;
...
pub fn ai_job_list(
  ...
  let (sql, param_values): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = if let Some(ref s) = status {
    (
      "SELECT id, skill_id, status, input_context, raw_response, actions, error, created_at, completed_at
