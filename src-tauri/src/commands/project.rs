...
#[tauri::command]
pub fn project_create(state: State<'_, AppState>, payload: ProjectCreatePayload) -> Result<ProjectRow, String> {
  let db = state.db().lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn project_update(state: State<'_, AppState>, payload: ProjectUpdatePayload) -> Result<(), String> {
  let db = state.db().lock().map_err(|e| e.to_string())?;
  let now = chrono::Utc::now().to_rfc3339();
