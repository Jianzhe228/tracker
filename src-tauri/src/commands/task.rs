...
pub fn task_update(state: State<'_, AppState>, payload: TaskUpdatePayload) -> Result<(), String> {
    ...
    let Some((current_due_at, current_recurring_rule_id, current_status, current_parent_id)) =
        current_task
    else {
    ...
    let mark_as_not_done = payload
        .status
        .as_deref()
        .is_some_and(|status| status != "done");
    let should_insert_completion_log =
        mark_as_done && current_status != "done" && current_status != "cancelled";
