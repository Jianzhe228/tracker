...
<template>
  <div class="flex h-full">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      ...
      <div class="border-b border-slate-200 bg-white px-6 py-4">
        <div class="flex items-center justify-between">
          ...
          <button class="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40" aria-label="更多操作">
        ...
        <div class="mt-4 grid grid-cols-3 gap-4 rounded-lg bg-gradient-to-r from-green-50 to-emerald-50 p-4">
          <div class="text-center">
            <div class="text-2xl font-semibold tabular-nums text-green-600">{{ completedCount }}<span class="text-sm text-slate-400">/{{ totalCount }}</span></div>
            <div class="text-xs text-slate-500">今日完成</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-semibold tabular-nums text-green-600">{{ completionRate }}%</div>
            <div class="text-xs text-slate-500">完成率</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-semibold tabular-nums text-amber-500">0</div>
      ...
      <div class="border-b border-slate-200 bg-white px-6 py-3">
        <div class="flex items-center gap-3">
          ...
          />
          <select aria-label="频率" class="rounded-lg border border-slate-200 px-2 py-1 text-xs text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40">
            <option>每日</option>
            <option>每周</option>
            <option>每月</option>
          </select>
        ...
        <div v-if="habitStore.habits.length > 0" class="space-y-2">
          ...
          >
            ...
            <button
              ...
              class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500/40"
              :class="habit.checkedToday
                ? 'bg-green-500 text-white'
                : 'border-2 border-slate-300 hover:border-green-400'"
              @click.stop="habitStore.checkHabit(habit.id)"
            >
              <svg v-if="habit.checkedToday" class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div class="flex items-center gap-1 text-xs text-slate-400">
              ...
              <span class="tabular-nums">0天</span>
            </div>
    ...
    >
      <aside v-if="selectedHabit" class="w-80 shrink-0 border-l border-slate-200 bg-white">
        <div class="flex h-full flex-col">
          <!-- Detail Header -->
          <div class="flex items-center gap-3 border-b border-slate-200 p-4">
            <button
              role="checkbox"
              :aria-checked="selectedHabit.checkedToday"
              :aria-label="selectedHabit.checkedToday ? '取消打卡' : '打卡'"
              class="flex h-6 w-6 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500/40"
              :class="selectedHabit.checkedToday
                ? 'bg-green-500 text-white'
                : 'border-2 border-slate-300'"
              @click="habitStore.checkHabit(selectedHabit.id)"
            >
              <svg v-if="selectedHabit.checkedToday" class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
              </svg>
            </button>
            <span class="min-w-0 flex-1 truncate font-medium text-slate-800">{{ selectedHabit.title }}</span>
          ...
          <div class="flex-1 overflow-auto p-4">
            <div class="space-y-4">
              ...
              <div class="flex items-center justify-between">
                ...
                <select aria-label="习惯频率" class="rounded-lg border border-slate-200 px-2 py-1 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/40">
                  <option>每日</option>
                  <option>每周</option>
                  <option>每月</option>
                </select>
              ...
              <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="rounded-lg bg-green-50 p-3 text-center">
                    <div class="text-xl font-semibold tabular-nums text-green-600">0</div>
                    <div class="text-xs text-slate-500">当前连续</div>
                  </div>
                  <div class="rounded-lg bg-amber-50 p-3 text-center">
                    <div class="text-xl font-semibold tabular-nums text-amber-600">0</div>
                    <div class="text-xs text-slate-500">最长连续</div>
                  </div>
                  <div class="rounded-lg bg-blue-50 p-3 text-center">
                    <div class="text-xl font-semibold tabular-nums text-blue-600">0</div>
                    <div class="text-xs text-slate-500">总打卡</div>
                  </div>
                  <div class="rounded-lg bg-slate-50 p-3 text-center">
                    <div class="text-xl font-semibold tabular-nums text-slate-600">0%</div>
                    <div class="text-xs text-slate-500">完成率</div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="rounded-lg border border-slate-100 p-3">
                  <div class="grid grid-cols-7 gap-1 text-center text-xs">
                    ...
                    <span v-for="d in 28" :key="d" class="rounded py-1 tabular-nums" :class="d === 8 ? 'bg-green-500 text-white' : 'text-slate-500'">
                      {{ d }}
                    </span>
                  </div>
                </div>
          ...
          <div class="border-t border-slate-200 p-3">
            <template v-if="!showDeleteConfirm">
              <button
                class="w-full rounded-lg border border-red-200 py-2 text-sm text-red-500 transition-colors hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/40"
                @click="requestDelete"
              >
                删除习惯
              </button>
            ...
            <template v-else>
              <p class="mb-2 text-center text-xs text-red-500">确定删除该习惯吗？此操作不可撤销</p>
              <div class="flex gap-2">
                <button
                  class="flex-1 rounded-lg border border-slate-200 py-2 text-sm text-slate-600 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40"
                  @click="cancelDelete"
                >
                  取消
                </button>
                <button
                  class="flex-1 rounded-lg bg-red-500 py-2 text-sm text-white transition-colors hover:bg-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/40"
                >
                  确认删除
                </button>
              </div>
