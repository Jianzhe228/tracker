...
- `src/services/commands/*.ts` 封装 Tauri `invoke()` 调用
- `src/services/suggestion/` 管理建议管线（pattern → learning → AI）
- `src/services/ai/` 管理 AI 调度（queue、prompt、client）

### Backend (Rust)
- `src-tauri/src/commands/` 暴露 Tauri commands
- `src-tauri/src/db/mod.rs` 管理 schema + migrations
- Migrations 使用 `PRAGMA user_version` 顺序递增
- 当前 schema version: v11
